...
class CustomLightningModule(BasicLightningModule):
    @staticmethod
    def _compute_control_loss_xz(pred_list, traj, traj_mask, traj_length, vae, device):
        """
        ...
        for i in range(len(pred_list)):
            pred_latent = pred_list[i].to(device)  # (T_token, 4)
        ...
        if "control_aux" in out and "traj" in batch:
            ...
            if control_weight > 0:
                ...
                loss_control = self._compute_control_loss_xz(
                    pred_list, traj, traj_mask, traj_length, self.vae, self.device
                )
