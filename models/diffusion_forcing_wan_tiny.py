...
class DiffForcingWanModel(nn.Module):
    def __init__(
        ...
        if lora_rank_traj is not None:
            warnings.warn(
                "`lora_rank_traj` is preferred; it overrides `traj_lora_rank`.",
                stacklevel=2,
            )
            traj_lora_rank = lora_rank_traj
        ...
        tlora = self.traj_lora_rank if self.use_traj_cond else 0
        self.model = WanModel(
            model_type="t2v",
            patch_size=(1, 1, 1),
        ...
        if freeze_backbone_for_traj:
            ...
            # 2) Freeze WanModel backbone; keep traj token path + traj-only LoRA trainable
