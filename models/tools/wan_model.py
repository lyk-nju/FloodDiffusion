...
class WanSelfAttention(nn.Module):
    def __init__(
        ...
        self.eps = eps
        self.causal = causal
        self.traj_lora_rank = traj_lora_rank
        # layers
        ...
        if traj_lora_rank > 0:
            ...
            self.lora_k_a = nn.Linear(dim, r, bias=False)
            self.lora_k_b = nn.Linear(r, dim, bias=False)
    ...
    def forward(
        ...
        Args:
            ...
            latent_pad_len: if set, x is [latent || traj] with L = 2 * latent_pad_len; traj segment
                starts at this index; traj-only LoRA applies to that slice. Self-attn 使用 Task4
                块稀疏（见 ``flextraj_self_attention``：默认可用双路 flash-attn，否则 SDPA）。
        """
        ...
        def qkv_fn(x_in):
            ...
            if (
                self.traj_lora_rank > 0
                and latent_pad_len is not None
                and s == 2 * latent_pad_len
            ):
                xt = x_in[:, latent_pad_len:, :]
                dq = self.lora_q_b(self.lora_q_a(xt)).view(b, -1, n, d)
                dk = self.lora_k_b(self.lora_k_a(xt)).view(b, -1, n, d)
                dv = self.lora_v_b(self.lora_v_a(xt)).view(b, -1, n, d)
                q = q.clone()
                k = k.clone()
                v = v.clone()
                q[:, latent_pad_len:, :, :] = q[:, latent_pad_len:, :, :] + dq
                k[:, latent_pad_len:, :, :] = k[:, latent_pad_len:, :, :] + dk
                v[:, latent_pad_len:, :, :] = v[:, latent_pad_len:, :, :] + dv
            return q, k, v
...
class WanAttentionBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = WanSelfAttention(
            dim, num_heads, window_size, qk_norm, eps, causal, traj_lora_rank
        )
        self.norm3 = (
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            traj_lora_rank (`int`, *optional*, defaults to 0):
                LoRA rank on traj-segment Q/K/V when traj_enc_dim > 0; 0 disables.
        """
        ...
        tlora = traj_lora_rank if traj_enc_dim > 0 else 0
        # embeddings
